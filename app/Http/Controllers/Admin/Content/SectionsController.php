...
class SectionsController extends Controller
{
    ...
    public function addSection(SectionAddRequest $request)
    {
        $request->validate([
            ...
            'slug.required' => 'Orice Sectiune trebuie sa aiba un slug unic',
            'slug.unique' => 'Orice Sectiune trebuie sa aiba un slug unic',
        ]);
        $section = new Section;
        ...
        Alert::success('A fost creata o noua sectiune', 'Sectiunea ' . $request->name . ' a fost creata cu succes!')
            ->persistent(true, false);
        return redirect(route('sections.list'))
            ->with('success', 'Sectiunea ' . $request->name . ' a fost adaugata in baza de date');
    }
    ...
    public function deleteSection($id)
    {
        ...
        if (!($section->photo == 'section.jpg')) {
            File::delete('content/section/' . $section->photo);
        }

        //$brand->detach();

        $section->delete();
        return redirect(route('section.list'))->with('success', 'Sectiunea ' . $section->name . ' a fost stearsa definitiv din baza de date');
    }
}
